...
const {ApiConmsoleCache} = require('./api-console-cache');
...
class ApiConsoleSources {
   ...
   * object that contains `log`, `info`, `warn` and `error` functions.
   * instance.
   */
  constructor(opts, githubResolver, transport, logger) {
    ...
    this._cache = new ApiConmsoleCache();
  }
  /**
  ...
  sourcesTo(destination) {
    // define a flow.
    const tag = this.opts.tagVersion;
  ...
  _copyLocal(from, to) {
    ...
    .then((stats) => {
      if (stats.isFile()) {
        ...
        message += 'Please, set this option to determine how to';
        message += ' handle the source file.';
